...
export async function sendRaportConducereEmbed({ username, userId, testName, rezultat, greseli, timpRamas, cooldownPana, intrebariGresite, esteAnticheat, motiv, screenshotBase64 }) {
  try {
    const now = new Date();
    const timeStr = now.toLocaleTimeString('ro-RO', {
      hour: '2-digit',
    ...
    // Dacă e anticheat și avem screenshot, trimitem cu imagine atașată
    if (esteAnticheat && screenshotBase64) {
      const base64Data = screenshotBase64.replace(/^data:image\/png;base64,/, '');
      const buffer = Buffer.from(base64Data, 'base64');

      embed.image = { url: 'attachment://anticheat.png' };
    ...
    // Fără screenshot — trimitere normală JSON
    const res = await fetch(process.env.DISCORD_WEBHOOK_CONDUCERE, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ embeds: [embed] }),
