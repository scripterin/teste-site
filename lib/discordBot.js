...
export async function sendRaportConducereEmbed({ username, userId, testName, rezultat, greseli, timpRamas, cooldownPana, intrebariGresite, esteAnticheat }) {
  try {
    const now = new Date();
    const timeStr = now.toLocaleTimeString('ro-RO', {
      hour: '2-digit',
    ...
    if (esteAnticheat) {
      fields.push({
        name: '⚠️ Anticheat',
        value: '**Candidatul a schimbat tab-ul în timpul testului.**',
        inline: false,
      });
    }
