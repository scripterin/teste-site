...
function TestBLSContent() {
  ...
  // Ref-uri pentru sincronizare instanta (esențiale pentru Beacon)
  ...
  // ✅ 1. BEACON: Trimite raportul DOAR la refresh/închidere tab
  useEffect(() => {
  ...
  const incarcaIntrebare = useCallback(async (index: number, isFirstLoad = false) => {
    ...
    try {
      ...
      if (res.status === 403) {
        // ✅ FIX: Dacă testul e deja marcat folosit (refresh anterior), oprim execuția dublă
  ...
  // ✅ 2. SUBMIT FETCH: Trimite raportul la finalizarea NORMALĂ a testului
  useEffect(() => {
    if ((stare === 'picat' || stare === 'promovat') && !submitting) {
      // Blocăm dacă nu avem cod SAU dacă motivul este refresh (pentru că a trimis deja Beacon-ul)
      if (!cod || motivFinal === 'refresh') return; 
  ...
  }, [stare, cod, submitting, motivFinal]);
  ...
  if (isInitialLoading) {
    return (
      <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#120E0C' }}>
  ...
  if (stare === 'picat' || stare === 'promovat') {
    ...
    return (
      <>
        ...
        <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#120E0C' }}>
  ...
  return (
    <>
      ...
      `}</style>
      <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 24px 40px', background: '#120E0C' }}>
          ...
          <div style={{ padding: '28px 28px 24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 20, borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom: 24 }}>
              <div>
                ...
                <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, letterSpacing: '0.05em', color: timpRamas < 60 ? '#C0392B' : '#F0EAE8', animation: timpRamas < 60 ? 'pulse 1s infinite' : 'none' }}>{formatTimp(timpRamas)}</p>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
              {intrebareCurenta?.optiuni.map((optiune, i) => {
                ...
                return (
                  <button key={i} className="opt-btn" disabled={!!feedback} onClick={() => setOptiuneSelectata(optiune)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, textAlign: 'left', background: isActive ? 'rgba(192,57,43,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${isActive ? '#C0392B' : 'rgba(255,255,255,0.07)'}`, backdropFilter: 'blur(8px)', cursor: feedback ? 'default' : 'pointer', width: '100%' }}>
                    <span style={{ flexShrink: 0, width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Mono', monospace", fontSize: 11, background: isActive ? 'rgba(192,57,43,0.2)' : 'rgba(255,255,255,0.05)', color: isActive ? '#C0392B' : 'rgba(255,255,255,0.3)' }}>{String.fromCharCode(65 + i)}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: '0.2em', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase' }}>{indexCurent + 1} / {totalIntrebari}</span>
                <button onClick={handleConfirm} disabled={!optiuneSelectata || !!feedback} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', fontFamily: "'DM Mono', monospace", fontSize: 10, background: (!optiuneSelectata || !!feedback) ? 'rgba(255,255,255,0.05)' : '#C0392B', color: (!optiuneSelectata || !!feedback) ? 'rgba(255,255,255,0.2)' : '#fff', cursor: (!optiuneSelectata || !!feedback) ? 'not-allowed' : 'pointer', boxShadow: (!optiuneSelectata || !!feedback) ? 'none' : '0 4px 16px rgba(192,57,43,0.3)' }}>
